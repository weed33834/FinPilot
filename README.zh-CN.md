...
**面向企业财务场景的开源智能体平台 · 自主推理 · 全链路可观测**

[![Language](https://img.shields.io/badge/文档-中文-green?style=flat-square)](#finpilot-ai)
...
- 财务计算严格走**确定性代码路径**（DCF、WACC、可比公司、回测），LLM 只负责叙事与解释；
- 每一次 API 调用、每一次问答、每一个模块开关都会写入**运行记录**，全程可回溯、可审计。
...
| 📱 **移动优先** | 独立的响应式移动端外壳（底部 Tab + 更多 Sheet），桌面专属页在小屏优雅降级 |
...
```mermaid
flowchart TB
  subgraph Client["接入层 — React 19 + Vite SPA"]
    Chat[AgentChatPage<br/>SlashCommandPalette · ReasoningChain · MarkdownRenderer]
    Admin[Admin / Reports / Audit / Settings]
    Mobile[Mobile Shell<br/>底部 Tab · 更多 Sheet · 响应式]
  end
  subgraph API["服务层 — FastAPI + LangGraph"]
...
- **接入层** — React 19 + Vite SPA，对话 / 报告 / 审计 / 管理多面板，实时 SSE 推送。
  ...
  - 独立的**移动端外壳**（`MobileShell`）提供触控优先导航；15+ 桌面专属管理页在小屏降级为友好的「请在桌面端打开」提示。
...
更深入的内容见 [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)。
...
sequenceDiagram
  ...
  AG-->>FE: thinking_token (计划 / 工具调用 / 观察结果)
  AG->>T: 调用工具 (nl2sql / document_qa / parse_document)
  T-->>AG: observation
  AG-->>FE: thinking_token (中间态)
  AG->>T: 调用工具 (最多 5 轮)
  AG-->>FE: answer_token (增量)
  AG-->>FE: done (推理链 + 置信度 + intent)
  Note over FE: 空闲时每 15s 发送心跳 "…"
```

ReAct 循环最多 **5 轮**工具调用，每个节点完成都通过 SSE 实时推送：
...
| 能力 | 说明 |
| :--- | :--- |
| **ABAC** | 基于属性的访问控制，按请求评估策略，而非仅按角色 |
| **TOTP 双因子** | RFC 6238 动态口令，支持二维码注册与备用码 |
| **PII 脱敏** | 自动识别并掩码日志与响应中的敏感字段 |
| **注入防护** | 对文档/工具输入做提示注入启发式检测 |
| **审计日志** | 每次特权操作的不可变记录 |
| **角色分级** | `admin` 与 `user`；后端对特权端点二次校验 |
...
```bash
cp .env.example .env          # 修改密钥（SECRET_KEY、FINPILOT_ADMIN_PASSWORD 等）
docker compose up -d          # Redis + PostgreSQL + 后端 + 前端
...
- 后端：`http://localhost:8010`（API + `/health` + `/metrics`）
- Redis：`6379` · PostgreSQL：`5432`

后端 `/health/ready` 供 Compose / K8s 的 `depends_on` 判定就绪。详细见 [docs/DEPLOYMENT.md](docs/DEPLOYMENT.md)。
...
│       ├── mobile/           # MobileShell / MobilePageHeader / *Mobile 页面
...
├── Dockerfile                # 容器构建定义
├── docker-compose.yml        # 一键编排（Redis + PG + 后端 + 前端）
├── setup.py                  # Python 包定义
...
| `FINPILOT_ADMIN_PASSWORD` | 默认管理员密码 | 未设置（不创建） |
...
| `FINPILOT_DATABASE_URL` | 数据库连接（覆盖 SQLite 默认） | SQLite |
| `REDIS_URL` | Redis（会话 / 限流 / 分布式锁） | — |
| `SECRET_KEY` | 会话加密密钥（≥32 字节，生产必填） | — |
...
- ✅ **v2.0.0** — 财务校验引擎、多智能体辩论、可解释性分析、风险分析、比率分析、三表建模（DCF/DDM/LBO 增强）、数据连接管理、回测增强、因子挖掘、MCP 服务器管理、技能/工具/提示词管理、沙箱配置、运行时日志、仪表盘（管理+用户）、**全量 i18n（en/zh-CN）**、**移动端响应式适配**
