"""
数据库连接模块 — 支持环境变量覆盖（PostgreSQL / SQLite）。

默认 SQLite（~/.finpilot/finpilot.db）；生产环境通过环境变量
``FINPILOT_DATABASE_URL`` / ``DATABASE_URL`` 切换到 PostgreSQL 等外部数据库。
...
# 优先读取环境变量覆盖数据库 URL（与 alembic/env.py 一致），
# 未设置则回退到本地 SQLite，保证开发环境开箱即用。
DATABASE_URL = (
    os.getenv("FINPILOT_DATABASE_URL")
    or os.getenv("DATABASE_URL")
    or ""
)

if not DATABASE_URL:
    # SQLite 默认路径：~/.finpilot/finpilot.db，自动创建目录
    DB_DIR = Path.home() / ".finpilot"
    DB_DIR.mkdir(parents=True, exist_ok=True)
    DATABASE_PATH = DB_DIR / "finpilot.db"
    DATABASE_URL = f"sqlite:///{DATABASE_PATH}"

# 不同方言采用不同引擎参数：SQLite 需关闭 check_same_thread + StaticPool 复用单连接；
# PostgreSQL/MySQL 等服务器型数据库使用默认连接池即可。
