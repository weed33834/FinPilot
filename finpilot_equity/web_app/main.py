...
LOGS_DIR = os.path.join(PROJECT_ROOT, "logs")  # 统一日志目录：finpilot_equity/logs/
...
from finpilot.database import init_db as init_finpilot_db
from finpilot.database.connection import SessionLocal as FinPilotSessionLocal
from finpilot.database.seed import seed_financial_data
...
async def list_all_logs(request: Request):
    ...
    # 只有管理员可以查看所有日志
    # Admin emails can be configured via FINPILOT_ADMIN_EMAILS env var (comma-separated)
    admin_emails = os.getenv("FINPILOT_ADMIN_EMAILS", "admin@finpilot.ai").split(",")
